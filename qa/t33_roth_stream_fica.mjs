...
const KNOWN_VERSIONS = ["v561", "v562", "v563", "v564", "v565", "v566", "v567", "v568", "v569", "v570", "v571", "v572", "v573", "v574", "v575", "v576", "v577"];
...
const POST_FIX = VER === "v563" || VER === "v564" || VER === "v565" || VER === "v566" || VER === "v567" || VER === "v568" || VER === "v569" || VER === "v570" || VER === "v571" || VER === "v572" || VER === "v573" || (VER === "v574" || (VER === "v575" || VER === "v576" || VER === "v577"));
...
const PINS = {
  ...
  // v5.65 populates CONNECTICUT's income-conditioned exemption. This suite's household is in
  // GEORGIA (`stateCode: "GA"`, L96), so neither figure may move — carrying v5.64's values forward
  ...
  // v5.67 populates NEW JERSEY's income-conditioned exclusion (N.J.S.A. § 54A:6-10) and models its
  // 62 age floor. This suite's household is in GEORGIA (`stateCode: "GA"`, L96), so neither figure
  // may move — carrying v5.66's values forward is the assertion that a populate release touches
  // only the state it populates. ⚠ v5.67 is the FIRST populate release to also change an AGE GATE,
  // which is a wider blast radius than a rule table: `exclAge` is read by the shared `_qual` for
  // every state. If either number has moved, the NJ floor has leaked into Georgia, which has no
  // exclAge at all and must still use the engine's 65 default.
  v567: { noStream: 174883, acaConv: 1203137 },
  // v5.68 populates VIRGINIA's age deduction. This household is in GEORGIA (`stateCode: "GA"`), so
  // neither figure may move — carrying v5.67's values forward is the assertion that the Virginia
  // change reached no other state. (The comments above cite L96 for that line; it is L118 at v5.68.)
  v568: { noStream: 174883, acaConv: 1203137 },
  v569: { noStream: 174883, acaConv: 1203137 },
  // v5.70 changes Ask AI's routing (SCOPE_B3_KEYLESS_AI_ROUTE) and touches no engine, tax or state rule, so neither figure
  // may move — carrying v5.69's values forward is the assertion that it did not. ⚠ THIS ENTRY WAS MISSED AGAIN at the v5.70
  // build, for the reason the v5.66 note above gives: the registration pass followed `vercensus.cjs`, which counts
  // STRING-literal tags and cannot see this identifier-keyed registry. The suite failed closed, as designed.
  v570: { noStream: 174883, acaConv: 1203137 },
  // v5.71 fixes the My Data draft autosave (A-3), guards an imported masterPrompt (A-6) and
  // rewords the first-open gate (F-3). It touches storage, the import path and copy — no engine,
  // no tax rule, no state rule — so neither figure may move. Carrying v5.70's values forward is
  // the assertion that it did not. ⚠ This entry is written BY HAND: vercensus.cjs sweeps
  ...
  // v5.74 shares the drawdown: Engine D publishes its spending-draw terms and ENGINES B AND C
  // consume them (C-8). Engine A — the Roth comparator this suite drives — is explicitly OUT OF
  // SCOPE (SCOPE_TAXES_DRAWDOWN D-7), so neither figure may move, and carrying v5.73's values
  // forward is the assertion that the release stayed inside its own scope rather than a copy.
  // If either number has moved, the draw bridge has leaked into Engine A and D-7 is no longer a
  // clean exclusion — read that as a finding, not as a pin to update.
  // ⚠ FOURTH consecutive release at which this registry had to be written BY HAND: its keys are
  ...
  // v5.76 fixes C-7: a SINGLE household is no longer paid spouse B's Social Security. This household is
  // MARRIED, so neither figure may move; carried forward as that assertion. If either moves, the fix
  // has reached a married household and the release is wrong, not this pin.
  v576: { noStream: 174883, acaConv: 1203137 },
  // v5.77 routes every engine's §86 through one helper and fixes Engine A's upper tier (C-4) and Engine C's
  // single-household thresholds (§1f). This household is MARRIED with benefits far above the line-14 region,
  // so neither figure may move; carried forward as that assertion.
  v577: { noStream: 174883, acaConv: 1203137 },
};
if (!PINS[VER]) {
