...
const KNOWN_VERSIONS = ["v548", "v549", "v550", "v551", "v552", "v553", "v554", "v555", "v556", "v557", "v558", "v559", "v560", "v561", "v562", "v563", "v564", "v565", "v566", "v567", "v568", "v569", "v570", "v571", "v572", "v573", "v574", "v575", "v576", "v577"];
...
const KEYS = [
  // Added v5.74 (C-8 / E-21 / D-14). Through v5.73 the Taxes and IRMAA tabs never taxed a spending
  // withdrawal and ran RMDs on a balance nothing was ever drawn from - one household projected two
  // ways, with the tab's own summary sentence asserting the opposite. Engine D now publishes the
  // draw and both tabs consume it. Direction: the correction LOWERS the headline (-$35,624, ~15%
  // on the example household), which is unusual in this project and is why it needs saying aloud.
  // WHY THIS KEY. "spending withdrawal", "drawdown", "RMD" and "capital gains" are each already on
  // one or both surfaces, so a key on the SUBJECT would pass before the fix existed - the vacuity
  // the v5.51, v5.52 and v5.54 controls each caught in turn. Measured against the PRISTINE v5.73
  // source and METHODOLOGY.md before any copy was written: this phrase returned 0 hits in both.
  // It names what CHANGED - that the tab carries the draw term at all.
  { key: "ordinary income your spending withdrawals create", since: "v574",
    why: "the Taxes and IRMAA tabs had no spending-draw term; both now import Engine D's" },
  ...
  { key: "reduced by Social Security", since: "v554",
    why: "the verified exclusions are reduced by Social Security and BOTH surfaces must name the mechanism" },
  // v5.56 — THE INVERSION the key above told this release to make. Its `why` used to end "and the
  // model applies none of it"; v5.56 applies it for MD and ME, so that clause was false the moment
  // the offset shipped and is corrected above. The MECHANISM key stays live and un-expired because
  // the disclosure is still owed: Colorado's shared cap is still unmodelled, and both statutes still
...
const POST = VER === "v549" || VER === "v550" || VER === "v551" || VER === "v552" || VER === "v553" || VER === "v554" || VER === "v555" || VER === "v556" || VER === "v557" || VER === "v558" || VER === "v559" || VER === "v560" || VER === "v561" || VER === "v562" || VER === "v563" || VER === "v564" || VER === "v565" || VER === "v566" || VER === "v567" || VER === "v568" || VER === "v569" || VER === "v570" || VER === "v571" || VER === "v572" || VER === "v573" || (VER === "v574" || (VER === "v575" || VER === "v576" || VER === "v577"));
...
const ORDER = ["v548", "v549", "v550", "v551", "v552", "v553", "v554", "v555", "v556", "v557", "v558", "v559", "v560", "v561", "v562", "v563", "v564", "v565", "v566", "v567", "v568", "v569", "v570", "v571", "v572", "v573", "v574", "v575", "v576", "v577"];
...
// that a list is missing. Roll all three together.
...
for (const k of KEYS) {
  ...
  if (expired) {
    ...
    // that legitimately predate it and stops the frozen leg replaying green (OPERATIONS §B2).
    T(`C [KNOWN DEFECT pre-v5.56]: "${key}" \u2014 the SS offset is disclosed and NOT applied`,
      !userSide(key), `app=${inApp(key)} docs=${inDocs(key)}`);
  } else if (since === "v553") {
...
if (POST) {
  ...
  if (VER === "v550" || VER === "v551" || VER === "v552" || VER === "v553" || VER === "v554" || VER === "v555" || VER === "v556" || VER === "v557" || VER === "v558" || VER === "v559" || VER === "v560" || VER === "v561" || VER === "v562" || VER === "v563" || VER === "v564" || VER === "v565" || VER === "v566" || VER === "v567" || VER === "v568" || VER === "v569" || VER === "v570" || VER === "v571" || VER === "v572" || VER === "v573" || (VER === "v574" || (VER === "v575" || VER === "v576" || VER === "v577"))) {
  ...
  if (VER === "v553" || VER === "v554" || VER === "v555" || VER === "v556" || VER === "v557" || VER === "v558" || VER === "v559" || VER === "v560" || VER === "v561" || VER === "v562" || VER === "v563" || VER === "v564" || VER === "v565" || VER === "v566" || VER === "v567" || VER === "v568" || VER === "v569" || VER === "v570" || VER === "v571" || VER === "v572" || VER === "v573" || (VER === "v574" || (VER === "v575" || VER === "v576" || VER === "v577"))) {
