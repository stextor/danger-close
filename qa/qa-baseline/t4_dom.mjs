...
const KNOWN_VERSIONS = ["v510", "v5101", "v5102", "v511", "v512", "v513", "v514", "v515", "v516", "v517", "v518", "v519", "v520", "v521", "v522", "v523", "v524", "v525", "v526", "v527", "v528", "v529", "v530", "v531", "v532", "v533", "v534", "v535", "v536", "v537", "v538", "v539", "v540", "v541", "v542", "v543", "v544", "v545", "v546", "v547", "v548", "v549", "v550", "v551", "v552", "v553", "v554", "v555", "v556", "v557", "v558", "v559", "v560", "v561", "v592"];
...
// ═══ v5.50 D-7 EXTINCTION — the estate objective discloses that it deducts no estate tax ═══
// WHY A COUPLE. The only estate-limitation text in the app before v5.50 sat inside a `_tlS.single`
// branch, so a COUPLE never rendered it — and couples are this app's primary audience and the
// household the comparator defaults to. A DOM test that only exercised a single filer would assert
// the disclosure exists while reproducing the exact defect that hides it. t31 cannot see this: it
// asks whether a string appears in the source of either user surface, and at v5.49 it already
// answered "yes" for `estate tax` on the strength of that single-gated card alone.
//
// So D7-0 is a GATE, not decoration. `widow's-penalty filing flip` renders only under `!P.single`
// and sits in the SAME paragraph as the narrowed noun, so it witnesses the household type at the
// point of measurement rather than somewhere else on the page. If it ever fails, every assertion
// below it is vacuous and the suite says so instead of going green.
//
// Gated to v550+: v5.48 and v5.49 legitimately still carry "after-tax estate", so an ungated
// assertion would fail both frozen legs — the v5.27→v5.28 defect (OPERATIONS §B2).
if (VER === "v552") {
  // ─── v5.52 D-10 · the ladder footnote discloses that its MAGI is narrower ───
  // The scope asks for single:false plus a !P.single witness IN THE SAME BLOCK. The Roth tab's
  // default fixture here is a couple, and D10-0 proves it at the point of measurement: the
  // ladder footnote's OWN couple branch is the survivor sentence one line above the new clause.
  // A tab-level witness elsewhere on the page would prove the household but not that THIS block
  // rendered its couple arm; a couple-blind fixture would then satisfy every check below while
  // rendering none of what they claim to test (OPERATIONS §B2 — the v5.49 estate-card case, where
  // a single-gated card answered "yes" for a household that never saw it).
  const rt10 = per["roth"] || "";
  T("D10-0 [FIXTURE GATE]: the LADDER FOOTNOTE rendered its couple branch — without this every check below is vacuous",
    /After the first projected death \(/i.test(rt10));
  T("D10-1: the footnote discloses that this MAGI omits dividends and realized capital gains",
    /it omits dividends and realized capital gains/i.test(rt10));
  T("D10-2: it says the figure reads too low, not merely that it differs",
    /reads a MAGI here that is too low/i.test(rt10));
  T("D10-3 [DIRECTION]: it names the direction of the error as optimistic",
    /the direction of that error is optimistic/i.test(rt10));
  T("D10-4 [SITE]: the clause renders under the IRMAA lookback footnote, beneath the table it qualifies",
    /IRMAA: 2-year lookback[\s\S]{0,900}it omits dividends and realized capital gains/i.test(rt10));
  T("D10-5: it routes the reader to the fuller figure rather than leaving a dead end",
    /treat the IRMAA tab as the fuller figure/i.test(rt10));
  // D-2: qualitative in-app, numbers in METHODOLOGY.md. A figure derived from one household goes
  // stale and nothing in-app carries a date — the same call made for D-7 and D-9.
  T("D10-6 [D-2]: no measured figure is shown to the user — the numbers live in METHODOLOGY.md",
    !/\$?45,?000/.test(rt10) && !/8 of 8/i.test(rt10));
}
if (VER === "v553" || VER === "v554" || VER === "v555" || VER === "v556" || VER === "v557" || VER === "v558" || VER === "v559" || VER === "v560" || VER === "v561") {
  // ─── v5.53 D-10 · the footnote reports a NARROWED gap, and the column carries a health warning ───
  // A new block, not an extended ladder: v5.53 rewrites the sentence v5.52 asserted, so extending
  // the gate would assert the pre-fix wording against the build that replaced it.
  //
  // t4 is the only suite that proves this RENDERS. t31 proves the same claim reaches both
  // surfaces and that the v5.52 wording is extinct on each; neither substitutes for the other,
  // because a clause can exist in the source and never reach the tab (OPERATIONS §B2).
  const rt10 = per["roth"] || "";
  T("D10-0 [FIXTURE GATE]: the LADDER FOOTNOTE rendered its couple branch — without this every check below is vacuous",
    /After the first projected death \(/i.test(rt10));
  T("D10-1 [HEALTH WARNING]: the IRMAA? column carries the dagger qualifier — an indication, not a verdict",
    /IRMAA\? column is an indication, not a verdict/i.test(rt10));
  T("D10-2 [SITE]: the dagger is on the COLUMN HEADER too, or the footnote marker points at nothing",
    /IRMAA\?\s*\u2020/.test(rt10));
  T("D10-3: the footnote says the column now counts the taxable sleeve's dividends",
    /now counts the taxable sleeve.{0,3}s dividends/i.test(rt10));
  T("D10-4 [PARTIAL FIX]: it names the term that REMAINS missing — a partial fix must not read as complete",
    /still missing/i.test(rt10) && /it omits realized capital gains/i.test(rt10));
  T("D10-5: it still says the figure reads low, not merely that it differs",
    /reads a MAGI here that is slightly too low/i.test(rt10));
  T("D10-6 [DIRECTION]: it still names the direction of the error as optimistic",
    /the direction of that error is optimistic/i.test(rt10));
  T("D10-7: it still routes the reader to the fuller figure rather than leaving a dead end",
    /IRMAA tab remains the fuller figure/i.test(rt10));
  T("D10-8 [EXTINCTION]: the v5.52 wording is gone from the rendered tab — the release that made it false removed it",
    !/it omits dividends and realized capital gains/i.test(rt10));
  T("D10-9 [D-2]: no measured figure is shown to the user — the numbers live in METHODOLOGY.md",
    !/\$?45,?000/.test(rt10) && !/8 of 8/i.test(rt10) && !/\$?342\b/.test(rt10));
}
if (VER === "v551" || VER === "v552" || VER === "v553" || VER === "v554" || VER === "v555" || VER === "v556" || VER === "v557" || VER === "v558" || VER === "v559" || VER === "v560" || VER === "v561") {
  // ─── v5.51 D-9 · the heir rate is disclosed as an assumption, to a COUPLE ───
  // Same reasoning as D-7's fixture gate: the comparator is a couple-first surface, and a
  // couple-blind fixture would assert the disclosure exists while never rendering the household
  // that needs it. The gate goes FIRST so a broken fixture fails loudly instead of passing vacuously.
  const rt9 = per["roth"] || "";
  T("D9-0 [FIXTURE GATE]: the roth tab is rendering a COUPLE - without this every check below is vacuous",
    /widow.{0,3}s-penalty filing flip/i.test(rt9));
  T("D9-1: the heir rate is disclosed as an assumption with no statutory source",
    /assumption with no statutory source/i.test(rt9));
  T("D9-2: the excluded STATE income tax is named specifically",
    /state.{0,40}income tax.{0,40}exclude/i.test(rt9));
  T("D9-3 [DIRECTION]: the note says the estate reads optimistic where the heir rate is higher",
    /where it is higher the estate shown here is/i.test(rt9) && /optimistic/i.test(rt9));
  T("D9-4: the rendered rate still reads 22% - v5.51 changed the disclosure, not the number",
    /22% assumed heir rate/i.test(rt9));
  T("D9-5 [D-2]: no numeric heir-rate range is shown to the user",
    !/13\s*[-\u2013]\s*31\s*%/.test(rt9));
}
if (VER === "v550" || VER === "v551" || VER === "v552" || VER === "v553" || VER === "v554" || VER === "v555" || VER === "v556" || VER === "v557" || VER === "v558" || VER === "v559" || VER === "v560" || VER === "v561") {
  const rt = per["roth"] || "";
  T("D7-0 [FIXTURE GATE]: the roth tab is rendering a COUPLE — without this every check below is vacuous",
    /widow.{0,3}s-penalty filing flip/i.test(rt));

  T("D7-1: the comparator note states no estate or inheritance tax is applied",
    /applies\s+no estate tax and no inheritance tax/i.test(rt));
  T("D7-2 [DIRECTION]: the note names the direction of the error as optimistic",
    /optimistic/i.test(rt));
  T("D7-3: the objective label is narrowed to what it actually computes",
    /MAX ESTATE AFTER HEIR INCOME TAX/i.test(rt));
  T("D7-4: the winner sentence uses the narrowed noun, not the old claim",
    /projects the largest estate after heir income tax/i.test(rt));
  T("D7-5: the comparator description uses the narrowed noun",
    /ranked by\s+ending estate after heir income tax/i.test(rt));
  // Found by D7-6 on its first run, at v5.50. The results table's COLUMN HEADER for the ranked
  // figure carried "After-tax estate" — capitalised, which is why the case-sensitive census greps
  // in both the scope and the build session missed it. It is the most prominent site of the five.
  T("D7-5b: the results-table column header is narrowed — the most prominent site of all",
    /Estate after heir income tax/i.test(rt));

  // The extinction invariant proper. "after-tax" asserts that tax has been accounted for, and for
  // estate and inheritance tax it has not been. If this string returns anywhere on the tab — in
  // the label, the noun, either literal sentence, or a new site — the claim is back and this fails.
  T("D7-6 [EXTINCTION]: the phrase 'after-tax estate' appears NOWHERE on the roth tab",
    !/after-tax estate/i.test(rt),
    (rt.match(/.{60}after-tax estate.{60}/i) || [""])[0]);
}
...
if (VER === "v524" || VER === "v525" || VER === "v526" || VER === "v527" || VER === "v528" || VER === "v529" || VER === "v530" || VER === "v531" || VER === "v532" || VER === "v533" || VER === "v534" || VER === "v535" || VER === "v536" || VER === "v537" || (VER === "v538" || (VER === "v539" || VER === "v540" || VER === "v541" || VER === "v541" || VER === "v542" || VER === "v543" || VER === "v544" || VER === "v545" || VER === "v546" || VER === "v547" || (VER === "v548" || VER === "v549" || VER === "v550" || VER === "v551" || VER === "v552" || VER === "v553" || VER === "v554" || VER === "v555" || VER === "v556" || VER === "v557" || VER === "v558" || VER === "v559" || VER === "v560" || VER === "v561")))) {
  ...
  if (VER === "v535" || VER === "v536" || VER === "v537" || (VER === "v538" || (VER === "v539" || VER === "v540" || VER === "v541" || VER === "v541" || VER === "v542" || VER === "v543" || VER === "v544" || VER === "v545" || VER === "v546" || VER === "v547" || (VER === "v548" || VER === "v549" || VER === "v550" || VER === "v551" || VER === "v552" || VER === "v553" || VER === "v554" || VER === "v555" || VER === "v556" || VER === "v557" || VER === "v558" || VER === "v559" || VER === "v560" || VER === "v561")))) {
...
if (VER === "v524" || VER === "v525" || VER === "v526" || VER === "v527" || VER === "v528" || VER === "v529" || VER === "v530" || VER === "v531" || VER === "v532" || VER === "v533" || VER === "v534" || VER === "v535" || VER === "v536" || VER === "v537" || (VER === "v538" || (VER === "v539" || VER === "v540" || VER === "v541" || VER === "v541" || VER === "v542" || VER === "v543" || VER === "v544" || VER === "v545" || VER === "v546" || VER === "v547" || (VER === "v548" || VER === "v549" || VER === "v550" || VER === "v551" || VER === "v552" || VER === "v553" || VER === "v554" || VER === "v555" || VER === "v556" || VER === "v557" || VER === "v558" || VER === "v559" || VER === "v560" || VER === "v561")))) {
  ...
  if ((VER === "v538" || (VER === "v539" || VER === "v540" || VER === "v541" || VER === "v541" || VER === "v542" || VER === "v543" || VER === "v544" || VER === "v545" || VER === "v546" || VER === "v547" || (VER === "v548" || VER === "v549" || VER === "v550" || VER === "v551" || VER === "v552" || VER === "v553" || VER === "v554" || VER === "v555" || VER === "v556" || VER === "v557" || VER === "v558" || VER === "v559" || VER === "v560" || VER === "v561")))) {
    const escAttrs = man.match(/[a-zA-Z-]+=\\+"/g) || [];
    if ((VER === "v539" || VER === "v540" || VER === "v541" || VER === "v541" || VER === "v542" || VER === "v543" || VER === "v544" || VER === "v545" || VER === "v546" || VER === "v547" || (VER === "v548" || VER === "v549" || VER === "v550" || VER === "v551" || VER === "v552" || VER === "v553" || VER === "v554" || VER === "v555" || VER === "v556" || VER === "v557" || VER === "v558" || VER === "v559" || VER === "v560" || VER === "v561"))) {
  ...
  if (VER === "v530" || VER === "v531" || VER === "v532" || VER === "v533" || VER === "v534" || VER === "v535" || VER === "v536" || VER === "v537" || (VER === "v538" || (VER === "v539" || VER === "v540" || VER === "v541" || VER === "v541" || VER === "v542" || VER === "v543" || VER === "v544" || VER === "v545" || VER === "v546" || VER === "v547" || (VER === "v548" || VER === "v549" || VER === "v550" || VER === "v551" || VER === "v552" || VER === "v553" || VER === "v554" || VER === "v555" || VER === "v556" || VER === "v557" || VER === "v558" || VER === "v559" || VER === "v560" || VER === "v561")))) {
...
if (VER === "v525" || VER === "v526" || VER === "v527" || VER === "v528" || VER === "v529" || VER === "v530" || VER === "v531" || VER === "v532" || VER === "v533" || VER === "v534" || VER === "v535" || VER === "v536" || VER === "v537" || (VER === "v538" || (VER === "v539" || VER === "v540" || VER === "v541" || VER === "v541" || VER === "v542" || VER === "v543" || VER === "v544" || VER === "v545" || VER === "v546" || VER === "v547" || (VER === "v548" || VER === "v549" || VER === "v550" || VER === "v551" || VER === "v552" || VER === "v553" || VER === "v554" || VER === "v555" || VER === "v556" || VER === "v557" || VER === "v558" || VER === "v559" || VER === "v560" || VER === "v561")))) {
...
// those builds legitimately carry.
{
  ...
  if (VER === "v531" || VER === "v532" || VER === "v533" || VER === "v534" || VER === "v535" || VER === "v536" || VER === "v537" || (VER === "v538" || (VER === "v539" || VER === "v540" || VER === "v541" || VER === "v541" || VER === "v542" || VER === "v543" || VER === "v544" || VER === "v545" || VER === "v546" || VER === "v547" || (VER === "v548" || VER === "v549" || VER === "v550" || VER === "v551" || VER === "v552" || VER === "v553" || VER === "v554" || VER === "v555" || VER === "v556" || VER === "v557" || VER === "v558" || VER === "v559" || VER === "v560" || VER === "v561")))) {
    ...
    if (VER === "v536" || VER === "v537" || (VER === "v538" || (VER === "v539" || VER === "v540" || VER === "v541" || VER === "v541" || VER === "v542" || VER === "v543" || VER === "v544" || VER === "v545" || VER === "v546" || VER === "v547" || (VER === "v548" || VER === "v549" || VER === "v550" || VER === "v551" || VER === "v552" || VER === "v553" || VER === "v554" || VER === "v555" || VER === "v556" || VER === "v557" || VER === "v558" || VER === "v559" || VER === "v560" || VER === "v561")))) {
...
// ═══ Verify tab reflects the version's check count ═══
{
  ...
  const _vCountV = (VER === "v532" || VER === "v533" || VER === "v534" || VER === "v535" || VER === "v536" || VER === "v537" || (VER === "v538" || (VER === "v539" || VER === "v540" || VER === "v541" || VER === "v541" || VER === "v542" || VER === "v543" || VER === "v544" || VER === "v545" || VER === "v546" || VER === "v547" || (VER === "v548" || VER === "v549" || VER === "v550" || VER === "v551" || VER === "v552" || VER === "v553" || VER === "v554" || VER === "v555" || VER === "v556" || VER === "v557" || VER === "v558" || VER === "v559" || VER === "v560" || VER === "v561")))) ? "66" : VER === "v531" ? "62" : _vCount;
...
if (VER === "v533" || VER === "v534" || VER === "v535" || VER === "v536" || VER === "v537" || (VER === "v538" || (VER === "v539" || VER === "v540" || VER === "v541" || VER === "v541" || VER === "v542" || VER === "v543" || VER === "v544" || VER === "v545" || VER === "v546" || VER === "v547" || (VER === "v548" || VER === "v549" || VER === "v550" || VER === "v551" || VER === "v552" || VER === "v553" || VER === "v554" || VER === "v555" || VER === "v556" || VER === "v557" || VER === "v558" || VER === "v559" || VER === "v560" || VER === "v561")))) {
...
if (VER === "v533" || VER === "v534" || VER === "v535" || VER === "v536" || VER === "v537" || (VER === "v538" || (VER === "v539" || VER === "v540" || VER === "v541" || VER === "v541" || VER === "v542" || VER === "v543" || VER === "v544" || VER === "v545" || VER === "v546" || VER === "v547" || (VER === "v548" || VER === "v549" || VER === "v550" || VER === "v551" || VER === "v552" || VER === "v553" || VER === "v554" || VER === "v555" || VER === "v556" || VER === "v557" || VER === "v558" || VER === "v559" || VER === "v560" || VER === "v561")))) {
  const NEW = VER === "v534" || VER === "v535" || VER === "v536" || VER === "v537" || (VER === "v538" || (VER === "v539" || VER === "v540" || VER === "v541" || VER === "v541" || VER === "v542" || VER === "v543" || VER === "v544" || VER === "v545" || VER === "v546" || VER === "v547" || (VER === "v548" || VER === "v549" || VER === "v550" || VER === "v551" || VER === "v552" || VER === "v553" || VER === "v554" || VER === "v555" || VER === "v556" || VER === "v557" || VER === "v558" || VER === "v559" || VER === "v560" || VER === "v561")));
