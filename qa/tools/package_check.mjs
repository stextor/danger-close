...
console.log("\nI. Scope status lines \u2014 candidates for retirement (reports, does not decide)");
{
  ...
  if (!existsSync(docs)) {
    ...
    const OPEN = new Set([
      ...
      //    the draws", not D's path copied — and it is NOT the A1 fallback this entry warned about. The entry's own
      //    expiry text ("consume Engine D's draw series and balance path") predates D-9, which is the stale-reason
      //    shape this block keeps recording, caught this time at removal. The scope carries its FULFILLED marker in its
      //    first 12 lines and its build record (§12), leaves the pool, and stays in docs/ — all in this same package.
      ...
      // ── REMOVED at the v5.72 ship (2026-09-15): "SCOPE_STATE_SET_SELECTOR.md" and "SCOPE_IMPORT_HARDENING.md",
      //    TOGETHER, as the second entry's own note required. Both entries named their expiry — the structural
      //    field shipping, and the build that retires the import scope — and v5.72 is both: it ships
      //    `incomeLimitedInLaw` (S-2) and H-1…H-6 with D-10. Both scopes carry RETIRED markers and §10 build
      //    records in this same package. The two halves shipped together.
      ...
      // scope ships RETIRED with a §10 build record in this same package.
      //   ⚠ THE TWO HALVES SHIPPED TOGETHER, as the SCOPE_TREE_AND_POOL_HOUSEKEEPING note below
      //   requires. Nothing here could have caught a missed removal: I-3 fires only on an entry
      //   naming a file that is GONE, and this file is still present carrying a RETIRED marker. The
      //   entry's own text said "a person removes it", and a person did.
