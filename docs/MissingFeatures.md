...
| **D-10** | Two MAGI figures render under one IRMAA label with no cue they are computed differently | ◑ **DISCLOSURE HALF SHIPPED at v5.52 (2026-08-27). MODELLING HALF SHIPPED IN PART at v5.53 (2026-08-28) — DIVIDENDS LANDED; REALIZED CAPITAL GAINS DID NOT, BY RESOLVED DECISION.** ⚠ **CORRECTED 2026-09-07 (Item A of `SCOPE_HOUSEKEEPING_THREE.md`). This row was stale for twelve releases and wrong in FOUR ways** — it said the modelling half was untouched, said *"dividends and realized capital gains remain absent"* when `_divLadder` had shipped, described the ladder as **five** terms when it is six, and pointed at `SCOPE_FIX_roth_tab_div_capgain.md` as the live route when that document is **SUPERSEDED and was never built**. **Where it stands, re-resolved BY AST against v5.65 `7604fac5dab891bb31905544d11072f8`, not by line number — the lines below have all moved at least once:** Engine C builds MAGI from **seven** terms (`ssTaxable + pen_y + work_y + rmdTax_y + conv_y + div_y + capGain_y`); the Roth ladder builds it from **six** (`pension + spouseBWork + taxableSS + conv_y + rmd_y + _divLadder`). **What is genuinely still open, and no more than this:** (1) **`capGain_y` is absent — and its absence is a RESOLVED DECISION, not an oversight.** `SCOPE_D10_MODELLING_v5_53.md` **D-2 (c)**: gains measured at **$342 across the entire ladder** on the example household and **$0** on both constructed ones, against real plumbing to hoist a series out of a closure. ⚠ **Do not re-open it; a register that re-litigates a settled call is the failure this row's own warning is about.** (2) **the earned-income term is narrower** — `spouseBWork` against Engine C's `work_y`, out of scope by the same release. Both can only ADD to MAGI, so the residual error runs one way. **The live route is `SCOPE_D10_MODELLING_v5_53.md` (RETIRED, shipped); `SCOPE_FIX_roth_tab_div_capgain.md` is SUPERSEDED — do not build from it.** ⚠ Its status line overstates: it reads *"BUILT AND SHIPPED"* while its title names both terms, and it is annotated in place to say so. **The product call is MADE and must not be re-litigated** — `SCOPE_D10_MAGI_DIVERGENCE_DISCLOSURE.md` (RETIRED) resolved it: disclose now, qualitative in-app, both surfaces. The ladder footnote and Field Manual §13 name the omitted terms and the direction; `METHODOLOGY.md` carries both expressions term by term; `t1` pins BOTH, which nothing did before; and the `IRMAA?` column carries a permanent *† an indication, not a verdict* qualifier. **What remains user-visible: the verdict can still be WRONG for a taxable-heavy household** — disclosed, not fixed. ⚠ `MEASUREMENT_roth_tab_magi_v5_40.md` recorded IRMAA **verdict inversions** in constructed households, so this is not cosmetic. Re-homed 2026-08-26 from the deleted `SCOPE_STRUCTURAL_MAGI_EXTINCTION.md`, where it was that scope's **D4** — *not* this register's D-4 | **Optimistic** — omitting income terms understates MAGI, understates the IRMAA trigger, and flatters the plan | Disclosure is a `src/` change and a version bump; needs its own scope | **Low** — the product call is made and the remaining term is a resolved decision; no open question ranks here |
...
| **D-11** | Three states' 65+ figures are right on one side of a statutory gate the data model cannot hold and wrong on the other (NM, RI, WI) | ◐ **RI's AGI cliff MODELLED at v5.69** (exclusive, TY2025 pair; its IRA exclusion moves to **D-12**) · **RI and WI amounts CORRECTED at v5.59** (20,000 → 50,000; 5,000 → 24,000) and their **67 age floors MODELLED at v5.60** (`exclAge: 67`, per person) — **WI is now exact on this provision**; **NM's income-conditioned exemption was MODELLED at v5.66** (NMSA 1978 § 7-2-5.2, nine bands, per person, conservative direction), leaving RI's AGI cliff and IRA exclusion as the only unmodelled gate | **MIXED for RI only** — NM and WI no longer flip; RI's sign still flips at its remaining gates (stated in its note) | Disclose now; RI's remaining gates need a data-model field | Medium |
| **D-12** | Account type — IRA versus employer plan — and per-person retirement income (Rhode Island's modification excludes every IRA and caps each person at their own income) | **Open — disclosed at v5.69** in RI's note, METHODOLOGY and the Field Manual | **Optimistic** for RI households with IRA money or one spouse holding the pension | **Build** — makes an existing output more correct for a mainstream couple (401(k)-to-IRA rollovers are common) | Med |
...
**NM was uncovered until v5.66**, which is what kept its half of D-11 open. It now carries the
largest cell block of the three: `t10` §2E prices EVERY band of both statutory tables to the dollar
(18 cells at the band tops, 14 one dollar past each top), plus the age floor, the per-person unit,
the AGI measure, the wage-spill clamp, an extinction invariant and the scalar-consistency check;
`t34` §A carries the structural table assertions and `t35` D-7a the guarded-set invariant. **NM's
half of D-11 is CLOSED as of v5.66.** Two limitations remain disclosed rather than modelled — the
table is unindexed since 1987, and blind claimants at any age cannot be expressed.

⚠ **D-11 is therefore now about TWO states, not three, and the heading above still says three.**
Left as-is deliberately: renaming a defect ID mid-life breaks every citation of it. The row and this
section state the current position.
...
the three**: `OTHER_TAX_TYPES` maps IRA, rollover, 401(k), 403(b), 457, pension, SEP and SIMPLE all to `trad`; positions carry
`trad`/`roth` dollars and an owner but no plan type; the pension input has no owner; and `stateTaxAnnual` receives household
totals only. v5.69 models the AGI cliff and **discloses** these three, each of which overstates the exclusion.

**Priced at scope time through `stateTaxAnnual`** (flat 5%, `ss` 0.5 held fixed): an $80,000 IRA-only couple is under-taxed
$4,000.00/yr; a couple whose $90,000 pension is all one spouse's, $2,000.00/yr; a couple where only the spouse under full
retirement age holds $80,000 of 401(k) income, $2,500.00/yr.

**Why it is its own item.** A plan-type field touches saved data (migration and defaults for every existing backup), every
traditional position and account, and how each engine sources a distribution — a data-model feature, not a populate. It
