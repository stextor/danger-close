...
**Session-only until Steve decides D-NM-1 below. Nothing shipped. No package cut.**
...
| This stop report | — | **Session-only unless (b) or (c) is chosen.** If either is, §3 and §4 should be re-homed into the new scope document, because **the finding is the durable part and a stop report is not** |
