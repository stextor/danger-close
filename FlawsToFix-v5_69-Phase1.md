...
| **Re-homed 2026-09-15 (ops, seventh) — two open follow-ups whose only pooled record was leaving the pool** | (1) **D-B3-1 (b)** — `docs/SCOPE_B3_KEYLESS_AI_ROUTE.md` chose (a) for v5.70: the keyless Ask-AI fix lives in the app, and **`src/main.jsx`'s `/anthropic` rewrite (L52–69, with `vite.config.js` L16–26 optionally) is left for its own later release.** That release has not been scoped. (2) **The two legacy `controls_v559` / `controls_v560` manifest rows contradict the manifest's own D-3 rule** — noted at several ships and deliberately left alone. Both were previously listed as open only in `SCOPE_PACKAGE_CHECK_BLIND_SPOTS.md`, which left the pool in that package (repo copy retained) |
| **Post-audit update, v5.73 (2026-09-15)** | **Both items filed below are RESOLVED at v5.73** (`SCOPE_ME_PHASEOUT_MT_CORRECTIONS.md`): the Montana lead was verified against the DOR's 2025 Form 2 and applied ($5,660), and Maine's phaseout is modelled. **Filed here now — three LEADS, not findings, each needing its own scope:** (1) **Maine's pension deduction has no age-65 test in law** (36 M.R.S. §5122(2)(M-2)); the model applies it from 65, which understates it before 65 — pessimistic, and doing it right needs the 59½ rule per account. (2) **Montana has two ordinary-income brackets**; the model uses a flat 5.65%, and the 2025/2026 bracket rates were not read from a primary source. (3) **The seven remaining `ss: 0.5` states (CO, CT, MN, NM, RI, UT, VT)** should each be checked against post-reform law — Montana's half-rate had described pre-2024 law for two years |
