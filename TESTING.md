...
**Current build: v5.13** · source `src/DangerClose.jsx` md5 `0ed9e140cd9163e4523d8ff71959d56c` · **482 automated checks green** against that exact source, plus 16 more against the built `index.html` (md5 `364fa4dcab05243b3c65b44a8b452d7e`).
...
### Current build (v5.13) — 267 checks in the t1–t6 baseline
...
### Cross-version and feature suites — 215 checks

| Suite | Checks | What it covers |
|---|---|---|
| t2 parity (v5.12 → v5.13) | 8 | Under common seeded random numbers with identical inputs, the Monte Carlo, extended MC, stress, and Roth engines produce **byte-identical** output across the two builds. This is the mechanical form of any "engines unchanged" claim |
...
**482 checks verify this build** = 267 (current leg) + 8 (parity) + 207 (t7–t9, t11–t14), every figure computed from parsed suite output rather than restated by hand. Frozen prior legs are re-proven at every run as history, and are expected to show since-fixed defects in their pre-fix state — that's correct, not a regression.

**Negative controls for this release.** t13 was run against the pre-fix v5.12 build, where it fails **14** assertions across all three omissions and both directions; the six substantive assertions that *pass* pre-fix are listed inside the file as not discriminating, so the suite is not counting them as wins. t14 fails **4** there, all of them the IRMAA engine — the other 20 pass because the Roth, Taxes and Withdrawal engines were already correct, which is what a class-level guard should look like once the class is closed.
